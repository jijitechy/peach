...
// Seller Boost Listing (Platform-funded Meta Ads)
app.post("/api/listings/:id/boost", async (req, res) => {
  ...
  const boostDays = parseInt(durationDays, 10) || 7;
  ...
  if (users[userIndex].balance < boostAmount) {
  ...
  // Deduct balance
  users[userIndex].balance -= boostAmount;
  await saveUser(users[userIndex]);

  // Update listing
  listing.isBoosted = true;
  listing.boostAmount = (listing.boostAmount || 0) + boostAmount;
  ...
  await saveListing(listing);
  await addActivity("ad_sync", `Seller ${user.name} boosted listing "${listing.title}" for ${boostAmount} (Live on Meta Ads)`);
